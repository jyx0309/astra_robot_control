...
class DualArmControlNode:
    def __init__(self, ip_addr="10.42.0.101", gui=False):
        rospy.loginfo("DualArmControlNode started.")
        ...
        rospy.loginfo("DualArmNode initialized. Waiting for connection command...")
        if not self.gui_enabled:
    ...
    def update_topics(self, topic_names):
        ...
        for key, (msg_type, cb) in self.sub_configs.items():
            ...
            self.subs_[key] = rospy.Subscriber(topic_name, msg_type, cb, queue_size=10)

        for key, msg_type in self.pub_configs.items():
            topic_name = topic_names.get(key, key)
            self.pubs_[key] = rospy.Publisher(topic_name, msg_type, queue_size=10)

        rospy.loginfo("Topics have been updated.")
    ...
    def connect_cb(self, msg: String):
        ...
        if not msg.data:
            self.carm_.disconnect()
            rospy.loginfo("connect: disconnect")
        else:
            self.carm_.connect(msg.data)
            rospy.loginfo(f"connect: {msg.data}")

    def ready_cb(self, msg: Bool):
        if self.carm_ is None: return
        self.carm_.set_ready()
        rospy.loginfo("set_ready")
    ...
    def set_left_eeff_cb(self, msg: JointState):
        ...
        if msg.position:
            ...
            rospy.loginfo(f"set_left_eeff, ret = {ret}")
    ...
    def set_right_eeff_cb(self, msg: JointState):
        ...
        if msg.position:
            ...
            rospy.loginfo(f"set_right_eeff, ret = {ret}")

    def set_control_mode_cb(self, msg: Int8):
        if self.carm_ is None: return
        ret = self.carm_.set_control_mode(msg.data)
        rospy.loginfo(f"set_control_mode, ret = {ret}")
...
class DualCarMGUI:
    def __init__(self, node):
        self.node = node
        self.root = tk.Tk()
        self.root.title("CARM Dual ROS 操作界面")
        ...
        # 话题配置区
        topics_frame = ttk.LabelFrame(self.root, text="ROS 话题配置 (修改后点击下方重启)")
    ...
    def connect(self):
        ...
        rospy.loginfo(f"UI trying to connect: {ip} ...")

        self.node.connect_device(ip)

        rospy.loginfo(f"UI connect finished: {ip}")

    def disconnect(self):
        rospy.loginfo("UI trying to disconnect ...")
        if self.node.carm_ is not None:
            self.node.carm_.disconnect()
        rospy.loginfo("UI disconnect finished")
...
def main(args=None):
    ...
    rospy.init_node("carm_dual_sdk", argv=ros_args)
