...
class ArmControlNode:
    def __init__(self, ip_addr="10.42.0.101", arm_index=0, gui=False):
        rospy.loginfo("ArmControlNode started.")
        ...
        rospy.loginfo("Node initialized. Waiting for connection command...")
        if not self.gui_enabled:
    ...
    def set_eeff_cb(self, msg: JointState):
        ...
        if msg.position:
            ...
            rospy.loginfo(f"set_eeff, ret = {ret}")
...
class CarMGUI:
    ...
    def connect(self):
        ...
        rospy.loginfo(f"UI trying to connect: {ip} (arm_index: {arm_idx}) ...")
...
def main(args=None):
    ...
    rospy.init_node("arm_control_sdk", argv=ros_args)
