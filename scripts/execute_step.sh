...
case "$kind" in
  set_gripper)
    ...
    ros2 action send_goal /robot/execute_step robot_interfaces/action/ExecuteStep \
      "{command_id: $command_id, kind: set_gripper, opening_m: $opening, force_n: $force, timeout_sec: 50.0}" --feedback
    ;;
  move_to|move_linear)
    [[ $# -ge 7 ]] || { echo "move 需要 X Y Z QX QY QZ QW" >&2; exit 2; }
    x="$1"; y="$2"; z="$3"; qx="$4"; qy="$5"; qz="$6"; qw="$7"; command_id="${8:-$command_id}"
    ros2 action send_goal /robot/execute_step robot_interfaces/action/ExecuteStep \
      "{command_id: $command_id, kind: $kind, timeout_sec: 50.0, target: {header: {frame_id: base_link}, pose: {position: {x: $x, y: $y, z: $z}, orientation: {x: $qx, y: $qy, z: $qz, w: $qw}}}}" --feedback
    ;;
